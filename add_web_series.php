	...
	if ($_SESSION['id'] != 2) {
		if (isset($_POST['submit']) and isset($_GET['add'])) {
			...
			$data = array(
				...
				'recommendation' => isset($_POST['recommendation']) ? 1 : 0,
				'trending' => isset($_POST['trending']) ? 1 : 0,
				'is_slider' => isset($_POST['is_slider']) ? 1 : 0,
				'w_trailer' => $_POST['w_trailer'],
				'spring_into_saturday' => isset($_POST['spring_into_saturday']) ? 1 : 0,
				'top_picks' => isset($_POST['top_picks']) ? 1 : 0,
				'w_status' => 1,
				'w_especially_date' => date("Y-m-d", strtotime($_POST['w_especially_date'])),
				'w_type' => $_POST['w_type'],
				'view_counts' => 1000000,
				'is_new' => isset($_POST['is_new']) ? 1 : 0,
				'is_new_content' => isset($_POST['is_new_content']) ? 1 : 0,
			);
					...
					<div class="page-header-content">
						...
						<div class="heading-elements">
							<div class="heading-btn-group">
								<a href="manage_web_series.php" class="btn btn-link btn-float has-text"><i
										class="icon-backward text-primary"></i><span>Back</span></a>
				...
				<div class="content">
					...
					<div class="panel panel-flat">
						<div class="panel-body">
							...
							<form class="form-horizontal" action="" name="addeditcategory" method="post"
								id="add_web_series_form" enctype="multipart/form-data" data-toggle="validator">
								<input type="hidden" name="spring_results" class="spring_results"
									value="<?php echo mysqli_num_rows($spring_results); ?>" />
								<input type="hidden" name="slider_results" class="slider_results"
									value="<?php echo mysqli_num_rows($slider_results); ?>" />
								<input type="hidden" name="w_id" value="<?php echo $_GET['w_id']; ?>" />
								<fieldset class="content-group">
									<div class="form-group">
										<label class="control-label col-lg-2">Type</label>
										<div class="col-lg-10">
											<select name="w_type" id="w_type" class="select-results-color form-control"
												required>
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
											<select name="cat_id" id="m_select1"
												class="select-results-color form-control" required>
												<option value="">--Select Category--</option>
												<?php
												while ($cat_row = mysqli_fetch_array($cat_result)) {
													?>
													<option value="<?php echo $cat_row['cid']; ?>" <?php if (isset($_GET['w_id']) && $cat_row['cid'] == $row['cat_id']) { ?>selected<?php } ?>><?php echo $cat_row['category_name']; ?>
													</option>
													<?php
												}
												?>
											...
											<div class="media no-margin-top">
												<div class="media-left">
													<div class="media-body">
														<input type="file" id="fileInput" name="w_image"
															class="file-styled-primary" accept="image/png, image/jpeg">
														</br>
														<?php if (isset($_GET['w_id']) and $row['w_image'] != "") {

															?>
															<img src="images/w_image/<?php echo $row['w_image']; ?>"
																style="width: 150px; " class="img-rounded" alt=""
																id="imagePreview"> <a
																class="remove_img btn-danger d-none"><i
																	class="glyphicon glyphicon-trash"></i></a>
														<?php } else { ?>
															<img src="assets/images/placeholder.jpg" style="width: 150px;"
																class="img-rounded" alt="" id="imagePreview" /> <a
																class="remove_img btn btn-danger d-none"><i
																	class="glyphicon glyphicon-trash"></i></a>
														<?php } ?>
														<span class="help-block">Accepted formats Jpg or png with
															dimensions 326x566</span>
													</div>
												</div>
											</div>
										</div>
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
											<input type="text" class="form-control" name="w_trailer" id="w_trailer"
												value="<?php if (isset($_GET['w_id'])) {
													echo $row['w_trailer'];
												} ?>"
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
											<input type="text" class="form-control" name="w_sub_title" id="w_sub_title"
												value="<?php if (isset($_GET['w_id'])) {
													echo $row['w_sub_title'];
												} ?>"
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
											<textarea class="form-control" name="w_description" id="w_description"
												placeholder="Description"><?php if (isset($_GET['w_id'])) {
													echo $row['w_description'];
												} ?></textarea>
										</div>
									</div>
									<div class="row">
										...
										<div class="col-lg-4">
											<input type="checkbox" name="recommendation" id="recommendation" value="1"
												<?php if (isset($_GET['w_id']) && 1 == $row['recommendation']) { ?>checked<?php } ?>> More Recommendation
										</div>
										<div class="col-lg-6">
											<input type="checkbox" name="top_picks" id="top_picks" value="1" <?php if (isset($_GET['w_id']) && 1 == $row['top_picks']) { ?>checked<?php } ?>>
											Top Picks
									...
									<div class="row">
										...
										<div class="col-lg-4">
											<input type="checkbox" name="is_new" id="is_new" value="1" <?php if (isset($_GET['w_id']) && 1 == $row['is_new']) { ?>checked<?php } ?>> New
											Release


										</div>
										<div class="col-lg-6">
											<input type="checkbox" name="trending" id="trending" value="1" <?php if (isset($_GET['w_id']) && 1 == $row['trending']) { ?>checked<?php } ?>>
											Most Trending
									...
									<div class="row">
										...
										<div class="col-lg-4">
											<input type="checkbox" name="is_new_content" id="is_new_content" value="1"
												<?php if (isset($_GET['w_id']) && 1 == $row['is_new_content']) { ?>checked<?php } ?>> New
										</div>
										<div class="col-lg-6">
										</div>
									</div>
									<div class="row">
										...
										<div class="col-lg-4">
											<input type="checkbox" name="spring_into_saturday" id="spring_into_saturday"
												value="1" <?php if (isset($_GET['w_id']) && 1 == $row['spring_into_saturday']) { ?>checked<?php } ?>> Spring into
											Saturday
										</div>
										<div class="col-lg-6">
											<input type="checkbox" name="is_slider" id="is_slider" value="1" <?php if (isset($_GET['w_id']) && 1 == $row['is_slider']) { ?>checked<?php } ?>>
											Display on Banner
									...
									<div
										class="form-group w_especially_date_section <?php if ($row['spring_into_saturday'] == 1) {
											echo '';
										} else {
											echo "d-none";
										} ?>">
										...
										<div class="col-lg-10">
											<input type="text" class="form-control date_picker" name="w_especially_date"
												id="w_especially_date" readonly
												value="<?php if (isset($_GET['w_id']) && $row['w_especially_date'] != '1970-01-01' && $row['w_especially_date'] != '0000-00-00') {
													echo date("d-m-Y", strtotime($row['w_especially_date']));
												} ?>"
												placeholder="Select date for spring into saturday">
										</div>
									</div>
									<div class="form-group">
										<label class="control-label col-lg-2">Main m3u8 Link :-</label>
										<div class="col-lg-10">
											<input type="text" class="form-control" name="main_m3u8" id="main_m3u8"
												value="<?php if (isset($_GET['w_id'])) {
													echo $row['main_m3u8'];
												} ?>" placeholder="Enter main m3u8 link">
										</div>
									</div>
									<div class="form-group">
								...
								<div class="text-right">
									<button type="submit" name="submit" class="btn btn-primary">Submit <i
